package hexgame;/* Copyright 2012 David Pearson.
 * BSD License
 */
...
public class Panel extends JPanel {
    ...
    public Panel() {
        ...
        if (colour != JOptionPane.CLOSED_OPTION) {
            opp = colour + 1;
            System.out.println("select colour:" + opp);
        } else {
//            colour = 2;
            System.out.println("application has done");
            System.exit(0);
        }
...
//            ((MCAI) ai).diffLevel = level.intValue();
            ((MCAI) ai).setDiffLevel(level.intValue());
        }
...
//        graph.fillRect(0, 0, 400, 500);
    ...
    private void drawBoard() {
        graph.setColor(new Color(110, 180, 140));
        graph.fillRect(0, 0, 400, 500);
