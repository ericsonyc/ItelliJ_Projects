...
public class CustomRecordWriter extends RecordWriter<Text, Text> {
    ...
    public void close(TaskAttemptContext context) throws IOException,
        ...
        String path = conf.get("path") + "/reduce/";
        int ont = conf.getInt("ont", 1);
        byte[] bytes = new byte[ont * Float.SIZE / 8];
        FSDataOutputStream fsOut = fs.create(new Path(filename), true);
        for (int i = 0; i < temp.size(); i++) {
            String[] strs = image.get(temp.get(i)).split("#");
            FSDataInputStream fsIn = fs.open(new Path(path + strs[0]));
            fsIn.seek(Long.parseLong(strs[1]));
            fsIn.read(bytes, 0, Integer.parseInt(strs[2]));
            fsIn.close();
            fsOut.write(bytes, 0, Integer.parseInt(strs[2]));
        }
        fsOut.close();
        System.gc();
    }
