...
class OutputShot {
    ...
    public OutputShot(int[] keys, float[] fcxy, ProgramConf pconf,
    ...
    public void getOuputShot(int[] keys, float[] cxcy) throws Exception {
        ...
        for (int i = 0; i < keys.length; ++i) {
            ...
            try {
                ...
                // System.exit(0);
                List<MyThread> threadLists = new ArrayList<MyThread>();
                int threadCount = 4;
                int threadLength = l / threadCount;
                for (int j = 0; j < threadCount; j++) {
                    int[] tempap = null;
                    if (j == threadCount - 1) {
                        tempap = new int[l - (threadCount - 1) * threadLength];
                    } else {
                        tempap = new int[threadLength];

                    }
                    for (int index = 0; index < tempap.length; index++) {
                        tempap[index] = ap[j * threadLength + index];
                    }
                    MyThread th = new MyThread(tempap.length, tempap, trace, i,
                            velocity, sxsy, gxgy);
                    threadLists.add(th);
                    th.start();
                }
    ...
    class MyThread extends Thread {
        ...
        private float[] sxsy = null;
        private float[] gxgy = null;

        // private HashMap<Integer, float[]> maps = null;

        public MyThread(int l, int[] ap, float[] trace, int i,
                        float[] velocity, float[] sxsy, float[] gxgy) {
            this.l = l;
            this.ap = ap;
            this.trace = trace;
            this.i = i;
        ...
        private void getOutput() {
            ...
            for (int t = 0; t < l; t++) {
                ...
                this.ktMigKernel(trace, i, velocity, ap[t], image, ox, oy, sx,
                        sy, gx, gy, nt, ont, ot, dt, oot, odt, maxtri, trfact,
                        beAntiAliasing);

                synchronized (maps) {
                    // HashMap<Integer, float[]> map = maps.get(ap[t]
                    // % maps.size());
        ...
        private void ktMigKernel(float[] trace, int traceindex, float[] vrms,
                                 ...
                                 float sy, float gx, float gy, int nt, int ont, float ot,
                                 float dt, float oot, float odt, int trm, float trf, boolean aa) {
            ...
            for (int k = 0; k < ont; ++k) {
                ...
                // squared distance to source from the image point on the
                // surface
                so2 = (sx - ox) * (sx - ox) + (sy - oy) * (sy - oy);
                // squared distance to receiver from the image point on the
                // surface
