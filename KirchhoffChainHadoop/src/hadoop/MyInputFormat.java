...
public class MyInputFormat extends FileInputFormat<IntWritable, Text> {
    ...
    public List<InputSplit> getSplits(JobContext job) throws IOException {
        // TODO Auto-generated method stub
        Configuration conf = ((Job) job).getConfiguration();
        Length = conf.getLong("FileSplitLength", 0);
        splitPerMap = conf.getInt("SplitPerMap", 1);
//        FileSystem fs=FileSystem.get(conf);
//        FSDataOutputStream out=fs.create(new Path(conf.get("path")+"config_tmp"),true);
//        out.writeLong(Length);
//        out.writeInt(splitPerMap);
//        out.flush();
//        out.close();
        System.out.println("splitPerMap:" + splitPerMap);
        System.out.println("length:" + Length);
        FileInputFormat.setMaxInputSplitSize((Job) job, Length);
...
//        lists.add(splits.get(0));
//        lists.add(splits.get(1));
        //return lists;
        return splits;
    ...
    private String[] getActiveServersList(JobContext context) {
        ...
        try {
            ...
            if (servers == null || servers.length == 0) {
                ...
                run.exec("hadoop job -kill " + context.getJobID());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    // 自定义的数据类型
    public static class ZRecordReader extends RecordReader<IntWritable, Text> {
        // data
...
//            int count = Integer.parseInt(buf);
        ...
        public boolean nextKeyValue() throws IOException, InterruptedException {
            ...
            System.out.println("nextKeyValuehello");
            ...
            System.out.println("value:" + data[0] + "," + data[data.length / 2] + "," + data[data.length - 1]);
            return true;
        }
