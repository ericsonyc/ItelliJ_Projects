...
def detectFaces(image_name):
    ...
    face_cascade = cv2.CascadeClassifier(
        "D:\\Program Files (x86)\\opencv\\opencv\\sources\\data\\haarcascades\\haarcascade_frontalface_alt2.xml")
    if img.ndim == 3:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
        gray = img  # if语句：如果img维度为3，说明不是灰度图，先转化为灰度图gray，如果不为3，也就是2，原图就是灰度图

    faces = face_cascade.detectMultiScale(gray, 1.3, 7)  # 1.3和5是特征的最小、最大检测窗口，它改变检测结果也会改变
    result = []
...
def drawFaces(image_name):
    ...
    if faces:
        ...
        for (x1, y1, x2, y2) in faces:
            ...
            draw_instance.line((x1,y1))
            draw_instance.rectangle((x1, y1, x2, y2), outline=(255, 0, 0))
        img.save("D:\\obama.jpg")
