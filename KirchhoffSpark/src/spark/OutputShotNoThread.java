...
public class OutputShotNoThread {
    ...
    public OutputShotNoThread(int[] keys, float[] fcxy, ProgramConf pconf,
    ...
    private void getOuputShot() throws Exception {
        ...
        this.readData(PATH + "/shot.data", nt, keys[0], Float.SIZE / 8,
                trace, fs);
        System.out.println("trace:" + trace[trace.length / 2]);
        this.readData(PATH + "/fsxy.data", 2, keys[0], Float.SIZE / 8,
                sxsy, fs);
        this.readData(PATH + "/fgxy.data", 2, keys[0], Float.SIZE / 8,
                gxgy, fs);
        ...
        for (int i = 0; i < keys.length; ++i) {
            ...
            try {
                ...
                quick_sort(ap, 0, l - 1);
                endTime = System.currentTimeMillis();
                System.out.println("sortTime:" + (endTime - startTime) + "ms");

                MyThread[] threads = new MyThread[1];
                for (int j = 0; j < threads.length; j++) {
                    int length = -1;
                    if (j == threads.length - 1) {
                        length = l - j * l / threads.length;
                    } else {
                        length = l / threads.length;
                    }
                    //System.out.println("length:" + length);
                    int[] app = new int[length];
                    for (int index = 0; index < length; index++) {
                        app[index] = ap[j * length + index];
                    }
                    //System.out.println("length:" + length);
                    MyThread th = new MyThread(app, 0, length, fs, trace, i, sx, sy, gx, gy);
                    threads[j] = th;
                    th.start();
                }
    ...
    class MyThread extends Thread {
        ...
        public MyThread(int[] ap, int index, int length, FileSystem fs, float[] trace, int key, float sx, float sy, float gx, float gy) {
            this.ap = ap;
            this.index = index;
            this.length = length;
            this.fs = fs;
        ...
        private void computeKernel() {
            float[] velocity = new float[ont * LENGTH];
            readData(PATH + "data/rmsv.data", ont, ap[0], Float.SIZE / 8, velocity, fs);
            int count = 0;
            for (int t = 0; t < length; t++) {
                ...
                float ox = oox + (ap[t + index * length] % onx) * odx;
                float oy = ooy + (ap[t + index * length] / onx) * ody;
                long sss = System.currentTimeMillis();
                //System.out.println("\tap[t]:" + ap[t]);
                //readData(PATH + "data/rmsv.data", ont, ap[t], Float.SIZE / 8, velocity, fs);
                if (ap[t] - ap[count] >= CONSTANT) {
                    if (length - t < LENGTH) {
                        LENGTH = length - t;
                    }
...
//                if (t % 1000 == 0)
//                    System.out.println("LENGTH:" + LENGTH);
                //System.out.println("velocity:" + velocity[velocity.length / 2]);
                long eee = System.currentTimeMillis();
                //System.out.println("\tvelocity Time:" + (eee - sss) + "ms");
                ktMigKernel(trace, key, velocity, ap[t] - ap[count], image, ox, oy,
...
//                }
                ...
                // result));
                //System.out.println("\tt:" + (t + index * length));
                image = null;
            }
        }
