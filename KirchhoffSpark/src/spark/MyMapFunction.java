...
public class MyMapFunction {
    ...
    private int apx, apy;
    private boolean beVerb, beDiff, beAntiAliasing;
    private float ot, dt, oot, odt, oox, ooy, odx, ody;
    private int onx, ony, ont, nt;
    private int maxtri;
    private float trfact;
    private ProgramConf pconf = null;
    private HashMap<Integer, float[]> maps = null;

    public MyMapFunction(ProgramConf pconf) {
        this.pconf = pconf;
        this.maps = new HashMap<Integer, float[]>();
    }

    public Iterable<Tuple2<Integer, String>> call(Iterator<Tuple2<Integer, String>> tuples)
            throws Exception {
        // TODO Auto-generated method stub
        List<Tuple2<Integer, String>> arrays = new ArrayList<Tuple2<Integer, String>>();
        List<Integer> keylists = new ArrayList<Integer>();
        List<Float> valuelists = new ArrayList<Float>();
        while (tuples.hasNext()) {
            Tuple2<Integer, String> tuple = tuples.next();
            int key = tuple._1();
            String[] values = tuple._2().split(",");
            for (int i = 0; i < values.length / 2; i++) {
                keylists.add(key++);
                valuelists.add(Float.parseFloat(values[2 * i]));
                valuelists.add(Float.parseFloat(values[2 * i + 1]));
            }
        }
        int[] keys = new int[keylists.size()];
        float[] fcxydatas = new float[valuelists.size()];
        for (int i = 0; i < keys.length; i++) {
            keys[i] = keylists.get(i);
            fcxydatas[2 * i] = valuelists.get(2 * i);
            fcxydatas[2 * i + 1] = valuelists.get(2 * i + 1);
        }
        ...
        try {
            ...
            this.getOuputShot(keys, fcxydatas);
