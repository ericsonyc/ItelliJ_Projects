...
public class CPUKTMigrationNoThread extends Mapper<IntWritable, Text, Text, Text> {
...
//			br.close();
    ...
    public void setMaps(int threads, int index) {
        THREADS = threads;
        Thread_Index = index;
    }
    ...
    public void map(IntWritable key, Text value, Context context)
        ...
        try {
            ...
            Configuration conf = context.getConfiguration();
            System.out.println(conf.get("nt"));
            int numberOfThreads = conf.getInt("ThreadNumbers", 1);
            int index = conf.getInt("Index", 0);
            int splitlength = keys.length / numberOfThreads;
            int[] keytemp;
            float[] fcxys;
            if (index == numberOfThreads - 1) {
                keytemp = new int[keys.length - index * splitlength];
                fcxys = new float[2 * keys.length];

            } else {
                keytemp = new int[splitlength];
                fcxys = new float[2 * keytemp.length];

            }
            System.out.println("keytemp.length:" + keytemp.length);
            System.out.println("keytemp:"+keytemp[0]);
            for (int i = 0; i < keytemp.length; i++) {
                keytemp[i] = keys[index * splitlength + i];
                fcxys[2 * i] = fcxydatas[2 * index * splitlength + i];
                fcxys[2 * i + 1] = fcxydatas[2 * index * splitlength + i + 1];

            }
            HashMap<Integer, float[]> maps = new HashMap<Integer, float[]>();

            OutputShot output = new OutputShot(keys, fcxydatas, context, maps);
            output.run();
            ...
            for (Entry<Integer, float[]> entry : maps.entrySet()) {
                ...
                context.write(new Text(String.valueOf(k)), new Text(result));
            }
