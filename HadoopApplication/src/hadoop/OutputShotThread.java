...
class OutputShotThread implements Runnable {
    ...
    public OutputShotThread(int[] keys, float[] fcxy, Context context,
    ...
    private void getOuputShot(int[] keys, float[] cxcy) throws Exception {
        ...
        this.readData(PATH + "data/shot.data", nt, keys[0], trace.length, Float.SIZE / 8,
                trace, fs, false);
        System.out.println("trace:" + trace[0] + "," + trace[trace.length / 2] + "," + trace[trace.length - 1]);
        this.readData(PATH + "data/fsxy.data", 2, keys[0], sxsy.length, Float.SIZE / 8,
                sxsy, fs, false);
        System.out.println("sxsy:" + sxsy[0] + "," + sxsy[1]);
        this.readData(PATH + "data/fgxy.data", 2, keys[0], gxgy.length, Float.SIZE / 8,
                gxgy, fs, false);
...
//            System.out.println("apx:" + apx + ",apy" + apy);
//            System.out.println("onx:" + onx + ",ony:" + ony);
//            System.out.println("cxcy:" + cxcy[2 * i] + ",cxcy:" + cxcy[2 * i + 1]);
//            System.out.println("oox:" + oox + ",ooy:" + ooy + ",odx:" + odx + ",ody:" + ody);
            int minix = onx - 1;
            int maxix = 0;
            int miniy = ony - 1;
            int maxiy = 0;
...
//                            % maps.length];
//                    if (!map.containsKey(ap[j])) {
//                        map.put(ap[j], String.valueOf(keys[i]));
//                    } else {
//                        String tt = map.get(ap[j]);
//                        map.put(ap[j], tt + "," + keys[i]);
//                    }
...
//                System.out.println("LENGTH:" + LENGTH);
                ...
                readData(PATH + "data/rmsv.data", datafully, ont, ap[0], LENGTH * ont, Float.SIZE / 8, velocity, fs, false);

                int count = 0;
                float[] image = new float[ont];
                for (int t = 0; t < l; t++) {
                    for (int ss = 0; ss < image.length; ss++) {
                        image[ss] = 0;
                    }
                    ...
                    if (ap[t] - ap[count] >= LENGTH) {
                        ...
                        readData(PATH + "data/rmsv.data", datafully, ont, ap[t], LENGTH * ont, Float.SIZE / 8, velocity, fs, false);
                        count = t;
