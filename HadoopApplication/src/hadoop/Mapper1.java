...
//            System.out.println("begin map");
            ...
            GetOutputShot outputShot = new GetOutputShot(keys, fcxydatas, context, maplists);
            outputShot.run();

//            HashMap<Integer, String>[] maplists = new HashMap[place];
//            for (int i = 0; i < place; i++) {
//                maplists[i] = new HashMap<Integer, String>();
//            }
...
//                Thread thread = new Thread(new MyOutputShot(keytemp, fcxy, context, maplists));
//                threads.add(thread);
//                thread.start();
//            }
//            keys = null;
...
//			}
            ...
            for (Entry<Integer, String> map : maplists.entrySet()) {
                //System.out.println("mapKey:" + map.getKey());
                //System.out.println("mapValue:" + map.getValue());
                context.write(new IntWritable(map.getKey()), new Text(map.getValue()));
            }

//            for (int i = 0; i < maplists.length; i++) {
//                HashMap<Integer, String> map = maplists[i];
...
//                    }
//                    fsOut.write(bytes, 0, vv.length * 4);
//                    fsOut.flush();
//                    context.write(new Text(String.valueOf(entryKey)), new Text(filename + "#" + offset + "#" + vv.length * 4));
